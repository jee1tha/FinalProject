...
public class BOLMethodsImpl implements BOLMethods{
	...
	public int RegisterUser(Applicants app) {
		...
		if(db.checkApplicantUsername(app) == true){
	...
	public int RegisterAdmin(Admin admin) {
		int result = 0;
		DatabaseMethodsImpl db = new DatabaseMethodsImpl();
		if(db.checkAdminUsername(admin)){
	...
	public ArrayList<Applicants> getApplicants(Applicants app) {
		...
		try {
			while(rs.next()){
				...
				ap.setBirthDate(rs.getString(""));
				ap.setContactNo(rs.getString(""));
				ap.setEmail(rs.getString(""));
				ap.setName(rs.getString(""));
				ap.setNicNo(rs.getString(""));
				ap.setPassword(rs.getString(""));
				ap.setRole(rs.getString(""));
				ap.setUsername(rs.getString(""));
	...
	public ArrayList<Job> getJobInfo(Job job) {
		...
		try {
			while(rs.next()){
				Job ap = new Job();
				ap.setJobDescription(rs.getString(""));
				ap.setName(rs.getString(""));
				ap.setJobid(rs.getInt(""));
